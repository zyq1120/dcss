...
CREATE TABLE document_extract_main (
    ...
    document_type VARCHAR(50) COMMENT 'document type (transcript, certificate, leave_application, etc.)',
    document_url VARCHAR(500) COMMENT 'document access URL (MinIO presigned URL)',
    raw_text LONGTEXT COMMENT 'OCR raw text content',
    basic_info_json JSON COMMENT 'basic info JSON (name, student_id, school, etc.)',
    academic_info_json JSON COMMENT 'academic info JSON (degree, enroll_date, etc.)',
    certificate_info_json JSON COMMENT 'certificate info JSON (cert_id, issue_date, etc.)',
    financial_info_json JSON COMMENT 'financial info JSON (bank_account, loan_amount, etc.)',
    leave_info_json JSON COMMENT 'leave info JSON (reason, days, etc.)',
    summary TEXT COMMENT 'document summary',
    tables_json JSON COMMENT 'table data JSON (courses, scores, etc.)',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CHECK (kv_data_json IS NULL OR JSON_VALID(kv_data_json)),
    CHECK (basic_info_json IS NULL OR JSON_VALID(basic_info_json)),
    CHECK (academic_info_json IS NULL OR JSON_VALID(academic_info_json)),
    CHECK (certificate_info_json IS NULL OR JSON_VALID(certificate_info_json)),
    CHECK (financial_info_json IS NULL OR JSON_VALID(financial_info_json)),
    CHECK (leave_info_json IS NULL OR JSON_VALID(leave_info_json)),
    CHECK (tables_json IS NULL OR JSON_VALID(tables_json)),
